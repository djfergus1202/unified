...
<title>BioMed Research Suite • Comprehensive Computational Biology Platform</title>
...
<style>
/* ... (All your CSS styles are correct, no changes needed) ... */
:root {
  --bg: #0a0e1a;
  --panel: #0f1419;
  --card: #1a1f2e;
...
const MolecularDockingModule = () => {
  ...
  // --- MODIFIED: Databases are now loaded from the backend ---
  const [proteinDatabase, setProteinDatabase] = useState({});
  const [ligandLibrary, setLigandLibrary] = useState({});

  // --- NEW: Load databases from backend on component mount ---
  useEffect(() => {
    const loadData = async () => {
      try {
        setError(null);
        const [proteinsRes, ligandsRes] = await Promise.all([
          fetch(`${API_BASE_URL}/docking/proteins`),
          fetch(`${API_BASE_URL}/docking/ligands`)
        ]);
        if (!proteinsRes.ok || !ligandsRes.ok) {
          throw new Error('Failed to fetch data from API');
        }
        const proteins = await proteinsRes.json();
        const ligands = await ligandsRes.json();
        setProteinDatabase(proteins);
        setLigandLibrary(ligands);
      } catch (err) {
  ...
  // --- MODIFIED: runDocking now calls the backend API ---
  const runDocking = async () => {
    ...
    try {
      ...
      if (json.success) {
        ...
        // Normalize backend data to fit frontend state
        const results = json.data;
        setDockingResults({
          ...results,
          bestAffinity: results.best_affinity, // Normalize key
          timestamp: new Date().toISOString(), // Add timestamp
          // The backend provides 'protein' and 'ligand' objects, which is perfect.
          // The backend 'modes' array matches the frontend's needs.
        });
  ...
  // This function is no longer used, as results come from API
  ...
  return (
    <div>
        ...
        <div className="grid grid-2">
          <div className="card">
            <div className="card-header">
              <h3 className="card-title">🧬 Protein Selection</h3>
            </div>
            <div className="form-group">
              ...
              <select 
                ...
                disabled={Object.keys(proteinDatabase).length === 0}
              >
                <option value="">
                  {Object.keys(proteinDatabase).length > 0 ? 'Choose protein...' : 'Loading proteins...'}
                </option>
                {/* --- MODIFIED: Use dynamic data --- */}
                {Object.entries(proteinDatabase).map(([id, protein]) => (
                  <option key={id} value={id}>
                    {protein.name} ({protein.pdb_id}) - {protein.organism}
                  </option>
                ))}
              </select>
            </div>

            {selectedProtein && proteinDatabase[selectedProtein] && (
              <div className="alert alert-info">
                <div>
                  <strong>Protein Details:</strong><br/>
                  Resolution: {proteinDatabase[selectedProtein].resolution} Å<br/>
          ...
          <div className="card">
            <div className="card-header">
              <h3 className="card-title">💊 Ligand Selection</h3>
            </div>
            <div className="form-group">
              ...
              <select 
                ...
                disabled={Object.keys(ligandLibrary).length === 0}
              >
                <option value="">
                  {Object.keys(ligandLibrary).length > 0 ? 'Choose ligand...' : 'Loading ligands...'}
                </option>
                {/* --- MODIFIED: Use dynamic data --- */}
                {Object.entries(ligandLibrary).map(([id, ligand]) => (
                  <option key={id} value={id}>
                    {ligand.name} - MW: {ligand.molecular_weight} g/mol
                  </option>
                ))}
              </select>
            </div>

            {selectedLigand && ligandLibrary[selectedLigand] && (
              <div className="alert alert-info">
                <div>
                  <strong>Ligand Properties:</strong><br/>
                  Molecular Weight: {ligandLibrary[selectedLigand].molecular_weight} g/mol<br/>
      ...
      <div className="card">
        <div className="card-header">
          <h3 className="card-title">⚙️ Docking Control</h3>
        </div>
      ...
      {/* Results Section */}
      {/* --- MODIFIED: Check for 'bestAffinity' which is the normalized key --- */}
      {dockingResults && activeTab === 'results' && (
        <div>
          ...
          <div className="grid grid-4">
            ...
            <div className="stat-card">
              {/* --- MODIFIED: Access binding_site object --- */}
              <div className="stat-value">{dockingResults.binding_site.volume}</div>
              <div className="stat-label">Pocket Volume (Å³)</div>
            </div>
            <div className="stat-card">
              <div className="stat-value">{dockingResults.binding_site.druggability}</div>
          ...
          <div className="card">
            <div className="card-header">
              <h3 className="card-title">📊 Binding Modes</h3>
            </div>
            <div className="table-container">
              <table>
                ...
                <tbody>
                  {/* --- MODIFIED: Use 'rmsd_lb' and 'rmsd_ub' from backend --- */}
                  {dockingResults.modes.map((mode) => (
                    <tr key={mode.mode}>
                      ...
                      <td>{mode.rmsd_lb.toFixed(2)}</td>
                      <td>{mode.rmsd_ub.toFixed(2)}</td>
          ...
          <div className="card">
            <div className="card-header">
              <h3 className="card-title">🔗 Protein-Ligand Interactions (Best Mode)</h3>
            </div>
            <div className="table-container">
              <table>
                <thead>
          ...
          <div className="card">
            <div className="card-header">
              <h3 className="card-title">📈 Binding Affinity Distribution</h3>
            </div>
...
const CellDynamicsModule = () => {
  const [selectedCellLine, setSelectedCellLine] = useState(''); // Changed default
  const [isSimulating, setIsSimulating] = useState(false);
  ...
  // --- MODIFIED: Load cell lines from backend ---
  const [cellLineDatabase, setCellLineDatabase] = useState({});
  
  // --- NEW: Load cell lines from backend on mount ---
  useEffect(() => {
    const loadCellLines = async () => {
      try {
        setError(null);
        const res = await fetch(`${API_BASE_URL}/cells/cell-lines`);
        if (!res.ok) {
          throw new Error('Failed to fetch cell lines');
        }
        const data = await res.json();
        setCellLineDatabase(data);
        // Set default selection
        if (Object.keys(data).length > 0) {
          setSelectedCellLine(Object.keys(data)[0]); // Set to first cell line
        }
      } catch (err) {
        console.error("Failed to load cell lines", err);
        setError(err.message);
      }
    };
    loadCellLines();
  ...
  const startSimulation = () => {
    // --- NOTE: This simulation remains CLIENT-SIDE ---
    // The backend API at '/api/cells/simulate' returns aggregate data for
    // the *entire* simulation run, which is incompatible with this
    // real-time canvas visualization that requires per-cell x/y coordinates
    // at each time step.
    //
    // To connect this, the backend API would need to be refactored
    // to support a streaming (WebSocket) or per-frame update.
  ...
  // Canvas rendering (no changes needed)
  useEffect(() => {
  ...
  // useMemo for stats (no changes needed)
  const stats = useMemo(() => {
  ...
  return (
    <div>
      ...
      <div className="card">
        <div className="card-header">
          <h3 className="card-title">🧫 Cell Line Selection</h3>
        </div>
        <div className="form-group">
          ...
          <select 
            ...
            disabled={Object.keys(cellLineDatabase).length === 0}
          >
            <option value="">
              {Object.keys(cellLineDatabase).length > 0 ? 'Choose cell line...' : 'Loading cell lines...'}
            </option>
            {/* --- MODIFIED: Use dynamic data --- */}
            {Object.entries(cellLineDatabase).map(([id, cell]) => (
              <option key={id} value={id}>
                {id} - {cell.origin} ({cell.type}) - DT: {cell.doubling_time}h
              </option>
            ))}
          </select>
        </div>

        {selectedCellLine && cellLineDatabase[selectedCellLine] && (
          <div className="alert alert-info">
            <div>
              ...
              Doubling Time: {cellLineDatabase[selectedCellLine].doubling_time} hours
              {/* The 'applications' field isn't in the backend model, so I'll remove it. */}
            </div>
          </div>
        )}
      </div>
      ...
      <div className="card">
        <div className="card-header">
          <h3 className="card-title">⚙️ Experiment Control</h3>
        </div>
        <button
          className="btn btn-primary"
        ...
        >
          {isSimulating ? (
            ...
            <>
              ▶️ Start Cell Culture Simulation
            </>
          )}
        </button>
        <div className="alert alert-warning" style={{marginTop: '1rem'}}>
          <strong>Note:</strong> The cell simulation is a client-side visualization.
          The backend API for cell simulation is incompatible with this real-time
          canvas component.
        </div>
      </div>

      {/* Live Visualization (no changes needed) */}
      <div className="card">
        <div className="card-header">
          <h3 className="card-title">👁️ Live Cell Culture</h3>
        </div>
        <canvas
          ref={canvasRef}
          width={800}
        ...
        <div style={{ textAlign: 'center', marginTop: '0.5rem', fontSize: '0.85rem', color: 'var(--muted)' }}>
          Scale: 1 pixel = 1 μm | Field of view: 800 × 600 μm
        </div>
      </div>
      ...
      {/* Growth Chart (no changes needed) */}
      {experimentData.length > 0 && (
        <div className="card">
          <div className="card-header">
            <h3 className="card-title">📈 Growth Curve</h3>
          </div>
          <ResponsiveContainer width="100%" height={300}>
